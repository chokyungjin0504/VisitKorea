...
import com.codingrecipe.member.repository.BoardFileRepository;
...
public class BoardService {
    ...
    public void saveB(BoardDTO boardDTO) throws IOException {
        // 파일 첨부 여부에 따라 로직 분리
...
//                MultipartFile boardFile = boardDTO.getBoardFile(); // 1. DTO에 담긴 파일을 꺼냄 -> 더이상 필요 없음
    ...
    public BoardDTO updateB(BoardDTO boardDTO) {
    ...
    public Page<BoardDTO> paging(Pageable pageable) {
        ...
        Page<BoardDTO> boardDTOS = boardEntities.map(board -> new BoardDTO(board.getId(), board.getBoardWriter(), board.getBoardTitle(), board.getBoardHits()));
        return boardDTOS;
    }
}
